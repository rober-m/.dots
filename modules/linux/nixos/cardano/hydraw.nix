...
in {
  options = {
    customized.cardano.hydraw.enable = lib.mkEnableOption "Enable cardano-node + hydra-node + example hydraw application";
  };

  config = lib.mkIf config.customized.cardano.hydraw.enable {
